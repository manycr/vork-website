...
export function CMSGrid({items,label="ver más"}:{items:CMSItem[];label?:string}){
 return <div className="grid gap-x-6 gap-y-16 md:grid-cols-3">
  {items.map((item)=><a key={item.id} href={hrefFor(item)} className="group block">
   <div
  className="aspect-[4/3] w-full bg-cover bg-center transition duration-700 group-hover:scale-[.985]"
