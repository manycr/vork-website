...
export default function Dashboard(){const[pw,setPw]=useState("");
const [authenticated,setAuthenticated]=useState(false);
const[tab,setTab]=useState<"items"|"leads">("items");const[items,setItems]=useState<CMSItem[]>([]);const[leads,setLeads]=useState<any[]>([]);const[form,setForm]=useState<any>(empty);const[edit,setEdit]=useState<string|null>(null);const[err,setErr]=useState("");const headers={"x-dashboard-password":pw,"Content-Type":"application/json"};
async function login(){
  setErr("");
  const r=await fetch("/api/dashboard/items",{headers});
  if(!r.ok){
...
if(!authenticated){
  return (
    <main className="min-h-screen bg-[#101010] px-[7vw] py-12 text-white">
      <div className="mx-auto mt-24 max-w-md">
        <p className="mb-4 text-xs font-black lowercase tracking-[0.08em] text-[#f2db9c]">
          panel privado
        </p>
        <h1 className="mb-8 text-5xl font-black lowercase tracking-[-0.06em]">
          vork cms privado.
        </h1>

        <input
          className="mb-3 w-full rounded-2xl border border-white/10 bg-white/10 px-5 py-4 text-white outline-none"
          type="password"
          placeholder="contraseña privada"
          value={pw}
          onChange={x=>setPw(x.target.value)}
          onKeyDown={x=>x.key==="Enter"&&login()}
        />
        ...
        <a href="/" className="mt-8 block text-sm font-bold text-white/60">
          volver al sitio
        </a>
      </div>
    </main>
