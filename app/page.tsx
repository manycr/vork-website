...
export default async function Home() {
  const projects = (await getPublishedItems("project", true)).slice(0, 3);
  const investments = (await getPublishedItems("investment", true)).slice(0, 5);
  const visuals = (await getPublishedItems("visual", true)).slice(0, 3);
  ...
  return (
    <main>
      ...
      <section className="bg-[#101010] px-[7vw] py-32 text-[#f4f0e8]">
        ...
        <CMSGrid items={investments} label="ver oportunidad" />
      </section>
      ...
      <section className="bg-[#101010] px-[7vw] py-32 text-[#f4f0e8]">
        ...
        <div className="grid gap-6 md:grid-cols-4">
          {[
            ["studio", "arquitectura, visualización y desarrollo conceptual.", "/studio"],
            ["build", "ejecución y construcción, próximamente.", "/build"],
            ["properties", "bienes raíces, próximamente.", "/properties"],
            ["investments", "oportunidades conceptuales para inversionistas.", "/investments"]
          ].map(([title, text, href]) => (
            <a key={title} href={href} className="min-h-72 border-t border-white/20 py-8 transition hover:opacity-70">
              <h3 className="text-4xl font-black lowercase tracking-[-0.06em]">vork<span className="font-light">{title}</span></h3>
              <p className="mt-6 text-sm leading-relaxed text-white/50">{text}</p>
            </a>
          ))}
        </div>
      </section>
